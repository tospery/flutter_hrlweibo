...
///
/// To integrate the [TextSpanField] into a [Form] with other [FormField] widgets,
...
///
/// {@tool sample}
/// This example shows how to create a [TextSpanField] that will obscure input. The
...
/// ## Reading values
///
/// A common way to read a value from a TextSpanField is to use the [onSubmitted]
/// callback. This callback is applied to the text field's current value when
/// the user finishes editing.
///
/// {@tool dartpad --template=stateful_widget_material}
///
/// This sample shows how to get a value from a TextSpanField via the [onSubmitted]
/// callback.
///
/// ```dart
...
/// callback.
///
/// Keep in mind you can also always read the current string from a TextSpanField's
...
///    [TextSpanField]. The [EditableText] widget is rarely used directly unless
...
class TextSpanField extends StatefulWidget {
  ...
  /// [TextSpanField.noMaxLength] then only the current length is displayed.
  ///
  ...
  /// to [TextSpanField.noMaxLength] then only the current character count is displayed.
  ///
  /// After [maxLength] characters have been input, additional input
  /// is ignored, unless [maxLengthEnforced] is set to false. The text field
  /// enforces the length with a [LengthLimitingTextInputFormatter], which is
  ...
  ///
  /// This value must be either null, [TextSpanField.noMaxLength], or greater than 0.
  /// If null (the default) then there is no limit to the number of characters
  /// that can be entered. If set to [TextSpanField.noMaxLength], then no limit will
  ...
  Widget build(BuildContext context) {
    ...
    Widget child = RepaintBoundary(
      child: EditableTextSpan(
        rangeStyles: widget.rangeStyles!,
        key: editableTextKey,
